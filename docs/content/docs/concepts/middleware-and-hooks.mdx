...
| `system` | `string \| undefined` | Yes | System prompt. Resolved from the agent's `system` option before middleware runs. |
...
| `context` | `C` | The request-scoped context passed via `agent.chat({ context })`. Typed to the agent's context schema. `undefined` if the agent has no context. See [Context](/concepts/context). |
...
When the agent has a context schema, `prepareStep` receives the typed context — useful for filtering tools based on user permissions or application mode:
...
const agent = createAgent({
  ...
  tools: { search, update_summary, update_questions, delete_record },
  memory,
  prepareStep: ({ context }) => {
    if (context.mode === "copilot") {
      return { activeTools: ["update_summary", "update_questions"] };
    }
    if (!context.permissions.includes("delete")) {
      return { activeTools: ["search", "update_summary", "update_questions"] };
    }
    return {};
  },
});
