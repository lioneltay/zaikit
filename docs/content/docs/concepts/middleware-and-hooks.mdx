...
Append a system message to `ctx.messages`. This keeps the system prompt stable for [prompt caching](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching) and makes the reminder ephemeral (it's never persisted — messages are already saved to memory before middleware runs).
