...
ZAIKit owns this loop directly rather than delegating it to the AI SDK. Each iteration makes a single-step `streamText()` call, giving the agent full control between steps for [middleware and hooks](/docs/concepts/middleware-and-hooks).
...
Use `onAfterStep` to enforce a maximum number of steps:
...
The manual loop exposes three levels of interception. See [Middleware & Hooks](/docs/concepts/middleware-and-hooks) for full details:
